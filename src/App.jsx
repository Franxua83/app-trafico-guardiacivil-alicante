...
// ✅ REGLA 4: RANGO SEMANA SANTA (Según instrucción estricta)
const SS_RANGE = { start: "2026-03-10", end: "2027-04-20" };

// ✅ REGLA 3: RANGOS NAVIDAD
...
// ✅ REGLA 3 & 4: Validaciones estrictas
const validateSpecialRange = (rangeDates, type) => {
  ...
  if (type === "nav") {
    // Comprobamos si el rango solicitado cae TOTALMENTE dentro de alguna de las ventanas de Navidad
...
// ✅ REGLA 5: Obtener última fecha de vacaciones para preferencia
const getLastVacationEnd = (requests, userId) => {
  const relevant = requests
    ...
    .map((r) => r.endDate)
    .sort(); // Ordena de más antiguo a más reciente
  // Si no tiene vacaciones previas, devolvemos una fecha muy antigua para darle prioridad
...
export default function AppFinal() {
  ...
  // ✅ REGLA 1: Detectar días cambiados para marcar en rojo
  ...
  const selectionStats = useMemo(() => {
    ...
    const workDays = range.reduce((acc, day) => {
      ...
      return SHIFT_STYLES[shift]?.workDay ? acc + 1 : acc;
    }, 0);
  ...
  // ✅ REGLA 5 CORREGIDA: Control de preferencia (Desplazamiento)
  const checkPreferenceAndDisplace = async (type) => {
    const displacements = new Set();
    const applicantLast = getLastVacationEnd(requests, currentUser.id);

    // Iteramos por cada día del rango solicitado
    for (const day of selectionStats.range) {
      // Filtrar solicitudes existentes en el grupo para ese día
      const groupRequests = requests.filter((r) => {
        const member = MEMBERS.find((m) => m.id === r.userId);
        return member && member.group === currentUser.group && r.days.includes(day) && ["vacation", "nav", "ss"].includes(r.type);
      });

      // Si el cupo está lleno para este día
      if (groupRequests.length >= MAX_VACATION_PER_GROUP) {
        
        // Ordenamos las solicitudes actuales para ver quién tiene MENOS preferencia.
        // Menos preferencia = Quien ha disfrutado vacaciones MÁS RECIENTEMENTE (fecha más grande/reciente).
        // Ordenamos descendente por fecha: el primero de la lista es el que tiene la fecha más reciente (el candidato a salir).
        const ranked = groupRequests
          .map((r) => ({ req: r, last: getLastVacationEnd(requests, r.userId) }))
          .sort((a, b) => b.last.localeCompare(a.last));

        const leastPreferred = ranked[0]; // Este es el que tiene la fecha más reciente (peor preferencia)

        // Comparamos: 
        // Si mi última fecha (applicantLast) es ANTERIOR (menor) a la suya (leastPreferred.last),
        // significa que yo llevo más tiempo sin vacaciones -> TENGO PREFERENCIA.
        if (applicantLast < leastPreferred.last) {
          displacements.add(leastPreferred.req.id);
        } else {
          // Si mi fecha es mayor o igual, no tengo preferencia.
          return { allowed: false, msg: `Cupo completo el día ${day} y no tienes preferencia sobre los existentes.` };
        }
      }
    }
  ...
  const processRequest = async (type, yearToCharge) => {
    ...
    // Validación especial (Navidad / SS)
    if (type === "nav" || type === "ss") {
    ...
    // Chequeo de Preferencias y Desplazamiento
    ...
    // Si hay desplazamientos, pedir confirmación explicita
    if (checkResult.displacements?.length) {
      const confirmDisplace = confirm("⚠️ AVISO DE PREFERENCIA\n\nEl cupo está lleno, pero llevas más tiempo sin disfrutar vacaciones que uno de los compañeros.\n\n¿Quieres DESPLAZAR al compañero con menos preferencia para ocupar su plaza?");
      if (!confirmDisplace) { 
        setModalError("Solicitud cancelada. No se ha modificado nada."); 
        return; 
      }
      
      // Ejecutar desplazamiento: borrar solicitudes anteriores
      try {
        ...
        await addDoc(collection(db, "artifacts", appId, "public", "data", "shift_chat_messages"), { 
            text: `⚠️ SISTEMA: ${currentUser.name} ha ejercido su derecho de preferencia por antigüedad en vacaciones, desplazando una solicitud existente.`, 
            userId: "system", 
            userName: "⚖️ PREFERENCIA", 
            groupId: currentUser.group, 
            createdAt: serverTimestamp() 
        });
      } catch(e) {
        console.error("Error borrando desplazamiento", e);
  ...
  const renderDay = useCallback((year, month, d) => {
    ...
    // ✅ REGLA 1: Detección visual de override
    ...
    return (
      <div
        ...
        } ${isOverrideDay ? "ring-2 ring-red-500 border-red-300" : ""}`}
      >
        ...
        {/* Marca visual explícita */}
        {isOverrideDay && (
          <div className="absolute inset-1 rounded-md border-2 border-red-500 pointer-events-none" aria-label="día cambiado" />
        )}
        {isOverrideDay && (
          <span className="absolute top-1 right-1 bg-red-100 text-red-700 text-[9px] font-black px-2 py-0.5 rounded-full shadow z-20">
            !
          </span>
        )}
              ...
              <div>
                ...
                {isWorkingDay ? (
                  <div className="space-y-2 mb-6">
                    ...
                    {/* Botones especiales con validación visual en botón */}
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-green-50">
              {messages.map((msg) => {
                const isSwap = msg.isSwap && msg.swapData;
                const canAccept =
                  isSwap && msg.swapData.status === "pending" && currentUser && msg.swapData.requesterId !== currentUser.id;

                return (
                  <div key={msg.id} className="border p-3 rounded bg-white text-xs shadow-sm">
                    {isSwap ? (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="font-bold text-blue-900">Solicitud de cambio</div>
                          <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 font-bold">Día {msg.swapData.date}</span>
                        </div>
                        <div className="text-[11px] text-slate-700 space-y-1">
                          <div><strong>Solicitante:</strong> {msg.swapData.requesterName}</div>
                          <div>
                            <strong>Turno:</strong> {msg.swapData.fromShift} → {msg.swapData.toShift}
                          </div>
                        </div>
                        {msg.swapData.status === "accepted" ? (
                          <div className="text-[11px] text-emerald-700 font-bold">✅ Cambio realizado por {msg.swapData.acceptedBy}</div>
                        ) : canAccept ? (
                          <button
                            onClick={() => acceptSwap(msg)}
                            className="w-full py-2 bg-emerald-600 text-white rounded-lg font-bold text-xs hover:bg-emerald-700"
                          >
                            ACEPTAR
                          </button>
                        ) : (
                          <div className="text-[11px] text-slate-500">Pendiente de aceptación...</div>
                        )}
                      </div>
                    ) : (
                      <div>{msg.text}</div>
            ...
            <form onSubmit={sendMessage} className="p-3 bg-white border-t flex gap-2">
              <input
                ...
                placeholder="Mensaje..."
              />
              <button type="submit" className="text-emerald-600"><Send /></button>
            </form>
          </div>
        </div>
      )}
