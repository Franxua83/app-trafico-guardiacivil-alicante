...
index ffcea95348230b5766866488c2dc1b3697389288..0e51bd165d4b759b423a1e22db4b04976466f64c 100644
--- a/src/App.jsx
+++ b/src/App.jsx
@@ -1,559 +1,892 @@
- (cd "$(git rev-parse --show-toplevel)" && git apply --3way <<'EOF' 
...
+};
+
+// Validaciones específicas para rangos especiales
...
+};
+
+// Devuelve la última fecha en la que el usuario disfrutó vacaciones (cualquier modalidad)
+const getLastVacationEnd = (requests, userId) => {
+  const relevant = requests
+    .filter((r) => ["vacation", "nav", "ss"].includes(r.type) && r.userId === userId && r.endDate)
+    .map((r) => r.endDate)
...
+  }, [requests, activeMembers]);
+
+  // Días que tienen cambios de servicio activos para el grupo en pantalla
+  const overrideMarkedDates = useMemo(() => {
+    const activeMemberIds = activeMembers.map((m) => m.id);
+    const set = new Set();
+    overrides.forEach((o) => {
...
+      });
+
+      if (["vacation", "nav", "ss"].includes(type) && groupRequests.length >= MAX_VACATION_PER_GROUP) {
...
+        } else {
+          return { allowed: false, msg: "No tienes preferencia para este cupo. Solicitud no permitida." };
+        }
+      }
+    }
+
...
+      const confirmDisplace = confirm("Tienes preferencia. ¿Quieres desplazar al guardia de menor preferencia para ocupar su plaza?");
+      if (!confirmDisplace) { setModalError("Solicitud cancelada por falta de confirmación."); return; }
...
+      }
+    });
+    if (targetGroups.length === 0) { alert("Ningún grupo de tu unidad tiene ese turno hoy por cuadrante."); return; }
+    const myCurrentShift = getEffectiveShift(dateStr, currentUser.id, currentUser.group);
+    const swapData = { type: "swap_request", requesterId: currentUser.id, requesterName: currentUser.name, date: dateStr, fromShift: myCurrentShift, toShift: targetShift, status: "pending", requesterGroupId: currentUser.group };
+    for (const gid of targetGroups) {
+      await addDoc(collection(db, "artifacts", appId, "public", "data", "shift_chat_messages"), { text: `🔄 SOLICITUD DE CAMBIO: ${currentUser.name} tiene ${myCurrentShift} el día ${dateStr} y busca ${targetShift}.`, userId: "system", userName: "📢 CAMBIO SERVICIO", groupId: gid, isSwap: true, swapData: swapData, createdAt: serverTimestamp() });
+    }
+    alert("Solicitud enviada al chat de los compañeros que trabajan ese día."); closeAllModals();
+  };
+
...
+    await addDoc(collection(db, "artifacts", appId, "public", "data", "shift_overrides"), { userId: msg.swapData.requesterId, date: msg.swapData.date, shift: msg.swapData.toShift, type: "swap", createdAt: serverTimestamp() });
+    await addDoc(collection(db, "artifacts", appId, "public", "data", "shift_overrides"), { userId: currentUser.id, date: msg.swapData.date, shift: msg.swapData.fromShift, type: "swap", createdAt: serverTimestamp() });
+    await updateDoc(doc(db, "artifacts", appId, "public", "data", "shift_chat_messages", msg.id), { "swapData.status": "accepted", "swapData.acceptedBy": currentUser.name });
+    if (msg.swapData.requesterGroupId) {
+      await addDoc(collection(db, "artifacts", appId, "public", "data", "shift_chat_messages"), { text: `✅ CAMBIO CONFIRMADO: ${currentUser.name} ha aceptado el cambio de turno de ${msg.swapData.requesterName} para el día ${msg.swapData.date}.`, userId: "system", userName: "✅ CAMBIO REALIZADO", groupId: msg.swapData.requesterGroupId, createdAt: serverTimestamp() });
+    }
...
+        )}
+        {isOverrideDay && (
+          <span className="absolute top-1 right-1 bg-red-100 text-red-700 text-[9px] font-black px-2 py-0.5 rounded-full shadow">
+            Día cambiado
+          </span>
+        )}
+        <div className="flex flex-col gap-0.5 mt-1 w-full overflow-hidden">
...
+                        </div>
+                        {msg.swapData.status === "accepted" ? (
+                          <div className="text-[11px] text-emerald-700 font-bold">✅ Cambio realizado</div>
+                        ) : canAccept ? (
+                          <button
